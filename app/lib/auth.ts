...
function sign(value: string) { return createHmac('sha256', getSessionSecret()).update(value).digest('base64url'); }
...
function getCookieValue(request: Request) {
  return request.headers.get('cookie')?.split(';').find((cookie) => cookie.trim().startsWith(`${SESSION_COOKIE_NAME}=`))?.trim().slice(SESSION_COOKIE_NAME.length + 1) ?? null;
}
function publicUser(user: User): Omit<User, 'passwordHash'> {
  const { passwordHash: _passwordHash, ...safeUser } = user;
  return safeUser;
}
export async function getPublicUserFromRequest(request: Request): Promise<Omit<User, 'passwordHash'> | null> {
...
export function createSessionToken(user: User) {
  const payload = encode(JSON.stringify({ sub: user.id, exp: Math.floor(Date.now() / 1000) + SESSION_MAX_AGE } satisfies SessionPayload));
...
export function clearSessionCookieOptions() { return { ...sessionCookieOptions(), maxAge: 0 }; }
